...
def vl_second_algorithm(peaks, new_peaks):
    ...
    return best_acc, peaks, new_peaks
...
def Dundee_data_run():
    result_df = pd.DataFrame(
        columns=['Free', 'With_ligand', 'vl_first', 'vl_second', 'optimizer', 'optimizer+post_proc'])

    this_path = pathlib.Path(__file__).resolve().parent

    for prot_ligand in experiments_dundee_V2:
        print(prot_ligand)
        assert len(prot_ligand) == 2
        print("==> FREE: ", prot_ligand[0], "| WITH LIGAND: ", prot_ligand[1])
        ...
        for alg_name, algorithm in algorithms:
            ...
            dd = {'Free': prot_ligand[0], 'With_ligand': prot_ligand[1]}
            print("LEN 1: ", len(peaks), "LEN 2: ", len(new_peaks))
            alg_res = algorithm(peaks, new_peaks)
            if alg_name == "vl_second":
                accuracy, p1, p2 = alg_res
            else:
                accuracy = alg_res
            #plotHistogram(p1, p2, prot_ligand[0], prot_ligand[1], alg_name, accuracy)
    ...
    result_df.to_csv(
        this_path.joinpath( "dundee_peak_shift_results_120121.csv"))
