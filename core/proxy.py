...
def optimizer_with_postprocessing(peaks, new_peaks, labeled):
...
def estimate_shifts(free_data,
    ...
    if assignment_algorithm == "SD":
        ...
        accuracy, p1, p2 = algorithm['RA'](peaks, new_peaks)        # okkkkkk
        p1.rename(columns={'AssignedTo': 'assigned_to'}, inplace=True)

    elif assignment_algorithm == "RASmart":                         # okkkkkk
        accuracy, p1, p2, w_half_size = algorithm['RASmart'](peaks, new_peaks, labeled)
        p1.rename(columns={'AssignedTo': 'assigned_to'}, inplace=True)
