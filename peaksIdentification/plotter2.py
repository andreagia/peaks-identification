...
def plotplot(spettro_old, spettro_new, associations):
    ...
    X = spettro_old[['x', 'y']].to_numpy()
    keys_X = spettro_old.index.tolist()

    Y = spettro_new[['x', 'y']].to_numpy()
    keys_Y = spettro_new.index.tolist()
    ...
    for triple in associations:
        ...
        old_p_xy = spettro_old.loc[key_old, ['x','y']].to_numpy(dtype=float)
        new_p_xy = spettro_new.loc[key_new, ['x','y']].to_numpy(dtype=float)
        ...
        fig.add_trace(go.Scatter(x=[old_p_xy[0], new_p_xy[0]],
                                 y=[old_p_xy[1]*0.2, new_p_xy[1]*0.2],
                                 mode='lines',
                                 showlegend=False,
                                 text='provaaa',
