...
from core.neighbor_based_assignment import get_avg_distances
...
#list_files=["MMP12_AHA_ref", "MMP12Cat_Dive_T1_ref_peaks"] # 2 test
...
pm = PeakManager(search_depth=1, max_search_per_level=3, log=False)
...
#xy_free, xy_with_ligands, associations = pm.getAssociations(old_spectra, new_spectra)

######################################################################################
print("-*"*50)
print(len(old_spectra.xy()), len(new_spectra.xy()) )

DistMatrix = distance_matrix(old_spectra.xy(), new_spectra.xy())
#print(DistMatrix)
...
DistMatrix = distance_matrix(peaks[['x', 'y']].to_numpy(), new_peaks[['x', 'y']].to_numpy())

#_cost, _assegnations = optimize_assegnation(DistMatrix)
_cost, _assegnations, accc = optimize_assegnation(peaks, new_peaks)

assigned_peaks = peaks[peaks['Distance']>-1]
cost2 = assigned_peaks['Distance'].sum()
acc2 = (np.array( assigned_peaks.index.tolist()) == np.array( assigned_peaks['AssignedTo'].tolist())).mean()

print("Cost: ", _cost, cost2)
print("Accuracy: ", accc, acc2)

# MODIFICA LA COST MATRIX CON LE NUOVE STRATEGIE / by Antonio, by me
# dividere ogni riga per il delta e ridare in pasto la matrice all'algoritmo
print(peaks)
for _ in range(10):
    print("///////////////////////////", _)
    #print(peaks['Distance'].tolist())
    avg = get_avg_distances(peaks, w_half_size=5, distance_col_name="Distance", keep_central=True)
    #print(DistMatrix)
    #print("Avg: ",avg)
    #print(type(avg), avg.shape)
    DistMatrix = DistMatrix / avg.reshape(-1, 1)
    #DistMatrix = DistMatrix
    #print(DistMatrix)
    _cost, _assegnations, accc = optimize_assegnation(peaks, new_peaks, distances=DistMatrix)
    print("Cost: ", _cost, " Acc: ", accc)
    #print(peaks)
